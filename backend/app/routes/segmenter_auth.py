...
from app.tasks.segmenter_task import run_segmenter
import uuid
import datetime as dt


segmenter_bp = Blueprint("segmenter", __name__, url_prefix="/api/segmenter")
...
@segmenter_bp.post("/start")
@jwt_required()
def start():
    uid = get_jwt_identity()
    data = request.get_json() or {}
    file_id = data.get("file_id")
    p = _new_progress(uid, "segmenter", file_id)
    run_segmenter.apply_async(args=[str(p.id), data], queue="segmenter")
    return jsonify({"progress_id": str(p.id), "status": "queued"}), 202


@segmenter_bp.get("/progress/<progress_id>")
@jwt_required()
...
@segmenter_bp.get("/results/<progress_id>")
@jwt_required()

...
export default function SegmenterTool({ fileId = null }) {
  ...
  const start = async () => {
    ...
    setResult(null);
    try {
      const resp = await fetch(`${config.API_BASE_URL}/segmenter/start`, {
        method: 'POST',
        headers: auth,
  ...
  useEffect(() => {
    ...
    let timer = setInterval(async () => {
      const r = await fetch(`${config.API_BASE_URL}/segmenter/progress/${progressId}`, { headers: auth });
      ...
      if (d.status === 'completed') {
        clearInterval(timer);
        const rr = await fetch(`${config.API_BASE_URL}/segmenter/results/${progressId}`, { headers: auth });

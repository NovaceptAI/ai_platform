...
def start():
    ...
    p = _new_progress(uid, "AI Presentation Builder", file_id)
    run_ai_presentation_builder.apply_async(args=[str(p.id), data], queue="ai_presentation_builder")
...
def results(progress_id):
    ...
    return jsonify({
        "summary": f"AI Presentation Builder placeholder result for progress={progress_id}",
        "chunks": [],
        "meta": {"version": 1},
    })
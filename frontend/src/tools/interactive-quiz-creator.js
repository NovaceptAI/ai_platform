...
import './interactive-quiz-creator.css';
import config from '../config';

export default function Interactive_quiz_creatorTool({ fileId = null }) {
  ...
  const start = async () => {
    ...
    setResult(null);
    try {
      const resp = await fetch(`${config.API_BASE_URL}/interactive-quiz-creator/start`, {
        method: 'POST',
        headers: auth,
  ...
  useEffect(() => {
    ...
    let timer = setInterval(async () => {
      const r = await fetch(`${config.API_BASE_URL}/interactive-quiz-creator/progress/${progressId}`, { headers: auth });
      ...
      if (d.status === 'completed') {
        clearInterval(timer);
        const rr = await fetch(`${config.API_BASE_URL}/interactive-quiz-creator/results/${progressId}`, { headers: auth });
  ...
  return (
    <div className="interactive-quiz-creator-tool">
      <div className="tool-head">
        <h3>Interactive Quiz Creator</h3>

...
export default function AiSummarizerTool({ fileId = null }) {
  ...
  const start = async () => {
    ...
    setResult(null);
    try {
      const resp = await fetch(`${config.API_BASE_URL}/ai-summarizer/start`, {
        method: 'POST',
        headers: auth,
  ...
  useEffect(() => {
    ...
    let timer = setInterval(async () => {
      const r = await fetch(`${config.API_BASE_URL}/ai-summarizer/progress/${progressId}`, { headers: auth });
      ...
      if (d.status === 'completed') {
        clearInterval(timer);
        const rr = await fetch(`${config.API_BASE_URL}/ai-summarizer/results/${progressId}`, { headers: auth });
  ...
  return (
    <div className="ai-summarizer-tool">
